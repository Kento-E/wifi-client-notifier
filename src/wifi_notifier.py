...
class WiFiRouter:
    ...
    def get_connected_devices(self) -> List[Dict[str, str]]:
        ...
        Returns:
            ...
            - 'device_type': デバイス種別（スマートフォン、PC、タブレットなど）
            - 'vendor': 製造元（MACアドレスから推定）
            - 'signal_strength': 信号強度
            - 'connection_speed': 接続速度
            - 'connection_time': 接続時刻
        """
        try:
...
class EmailNotifier:
    ...
    def _create_email_body(self, device_info: Dict[str, str]) -> str:
        ...
        if device_info.get("vendor"):
            additional_info.append(f"製造元: {device_info['vendor']}")
            has_additional_info = True

        if device_info.get("device_type"):
            additional_info.append(f"デバイス種別: {device_info['device_type']}")
            has_additional_info = True

        if device_info.get("user_agent"):
            additional_info.append(f"ユーザーエージェント: {device_info['user_agent']}")
            has_additional_info = True

        if device_info.get("signal_strength"):
            additional_info.append(f"信号強度: {device_info['signal_strength']}")
            has_additional_info = True

        if device_info.get("connection_speed"):
            additional_info.append(f"接続速度: {device_info['connection_speed']}")
            has_additional_info = True

        if device_info.get("connection_time"):
            additional_info.append(f"接続時刻: {device_info['connection_time']}")
            has_additional_info = True

        # 追加情報がある場合は本文に追加
        if has_additional_info:
            body_parts.append("")
            body_parts.append("【詳細情報】")
            body_parts.extend(additional_info)

        # フッター
        body_parts.extend(["", "---", "WiFi Client Notifier"])
        ...
        try:
            ...
            for mac in new_macs:
                ...
                if device_info:
                    ...
                    if should_notify:
                        # デバイス情報をログに記録
                        device_details = f"{mac}"
                        if device_info.get("hostname"):
                            device_details += f" ({device_info['hostname']})"
                        if device_info.get("vendor"):
                            device_details += f" - {device_info['vendor']}"
                        if device_info.get("device_type"):
