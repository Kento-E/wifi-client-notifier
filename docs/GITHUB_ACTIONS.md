...
| `STATE_FILE` | 検知状態を保存するファイル名 | `wifi_notifier_state.json` |
...
- `STATE_FILE` に端末識別情報（MACアドレス）が含まれることを前提に、公開リポジトリではキャッシュ運用を慎重に設計する
- 必要に応じて、キャッシュの利用範囲を限定（ブランチ/環境）し、不要な場合は状態ファイルキャッシュ自体を無効化する
...
- Secretsの値をログに出力
- 本番用パスワードを共有リポジトリで使用
- 公開範囲や運用ポリシーを確認せずに、端末識別情報を含む状態ファイルをキャッシュ保存する
